...
- `FormatterService` owns conservative editor formatting reports, including indentation, simple declaration-block alignment, simple ANSI port-list alignment, simple named instance-map alignment, trailing `//` comment preservation for those aligned blocks, and full-line selection formatting with base-indent preservation. The editor invokes it and applies the returned text as a single undoable edit, while formatter policy stays outside UI code.
