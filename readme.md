...
Editor source-navigation hit-testing now lives behind `SourceNavigationService`.
`MyCodeEditor` asks the service for include paths and package imports, then keeps only cursor mapping, highlighting, and jump/open dispatch.
...
- `SourceNavigationService` owns SystemVerilog include directive and package-import hit-testing for editor source navigation.
