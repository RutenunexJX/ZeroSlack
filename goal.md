...
- I4 is current: delete the legacy carrier APIs, compatibility taxonomy surface, and remaining adapter conversions. The unused `semanticcollectoradapter` source/header are deleted and guarded against returning, `SemanticIndex` no longer exposes or stores a legacy `sym_list` database injection/access API, dead `sym_list` symbol database mutator/accessor/index/scope-tree storage has been deleted, native taxonomy metadata checks no longer route through the retired private raw-kind bridge, unused legacy metadata write-back helpers have been removed from the taxonomy compatibility surface, legacy `SymbolInfo` owner/visibility/interface/declaration helper overloads are no longer public taxonomy API, and legacy `SymbolInfo` scope/visibility helper APIs have been deleted in favor of the native semantic metadata definition-visibility API.
