...
- I3 is current: scope rebuild, module containment, module lookup, and relationship containment should use semantic metadata, owner/type records, stable keys, and explicit local handles instead of `symbolId`, `symbolType`, `moduleScope`, or `dataType`. First block complete: relationship builder input is supplied by `SemanticIndex` semantic records instead of legacy `sym_list` record conversion.
