...
- I4 is current: delete the legacy carrier APIs, compatibility taxonomy surface, and remaining adapter conversions. The unused `semanticcollectoradapter` source/header are deleted and guarded against returning, and `SemanticIndex` no longer exposes or stores a legacy `sym_list` database injection/access API.
