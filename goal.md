...
- I2 is in progress: `SemanticIndex` now owns native symbol records, file contents, file-state hashes, file coverage, local handles, stable-key indexes, and native-over-snapshot replacement before bridging to the remaining `sym_list` compatibility layer.
