...
- Wave Preview remains a code-understanding sketch rather than a simulator. Formatter remains conservative, profile-driven, and source-preserving. Huge Workspace Mode remains priority/cancellation/provenance driven, with staged symbol visibility, explicit current/open/background band semantics, a Release-measured full relationship warm-up path, and stage telemetry for workspace symbol publication.
...
- Ctrl+Space opens a centered ZeroSlack Global Control overlay from editor or non-editor focus; the current command surface is domain-first with root domains `ow` and `fd`, where `ow <num>` opens workspaces and `fd r` / `fd s` enter Fold Region or Fold Shelf mode. Broader file/symbol/template/action search remains reserved for later expansion.
...
### Post-L: Huge Workspace Visible Completion Band Summary Header MVP

- Status: implemented in the current worktree.
- Scope: thirty-sixth usable Huge Workspace Mode milestone and an ordinary completion provenance accuracy refinement. It does not change semantic extraction, completion scoring, completion truncation limits, command-symbol completion behavior, diagnostic publication, or workspace scheduling.
- `CompletionModel::updateCompletions()` now builds its completion-band header from visible, real symbol rows after scoring and truncation instead of using the full `CompletionResult` summary directly.
- `CompletionResult::analysisBandSummaryText()` remains the full-result service/report summary, so non-UI callers can still inspect hidden background provenance when they need it.
- The ordinary completion popup now omits the `:: COMPLETION BANDS - ... ::` header when lower-priority background rows exist in the full result but are not visible after truncation.
- `completion_test` verifies that the full result still reports both current and hidden background bands while the rendered popup stays at the 15 visible rows with no misleading band header.
- Implementation remains no-regex and keeps priority policy in `WorkspaceAnalysisPlanService` / `SemanticIndex`; completion UI consumes metadata already attached to completion items and does not recompute workspace priority tiers.
- Next Huge Workspace milestones: per-band diagnostics publication only if semantically safe, richer tiered index publication, and additional completion/report surfaces that consume analysis-band provenance without recomputing priority policy.
- Verification for this block: focused build target `completion_test`; direct `completion_test` run with 563 checks. Final release verification for the commit also includes changed-file C++ regex API scan, full default CMake build, full `ctest --output-on-failure` 7/7, and `git diff --check`.

### Post-L: Huge Workspace Relationship Performance MVP

- Status: implemented in the current worktree.
- Scope: thirty-seventh usable Huge Workspace Mode milestone and the first direct performance fix for full workspace relationship warm-up. It keeps automatic workspace relationship analysis after full workspace symbol analysis; the policy remains full upfront analysis, but the implementation is now bounded by indexed snapshot and relationship-context lookups.
- Baseline: `test_sv/huge_prj` has 428 HDL files and 18.59 MiB of source. Symbol/diagnostic indexing was not the main blocker: prior GUI measurements were 46.691s Release and 53.448s Debug for the symbol stage, close to DVT Eclipse's 47s reference. The blocking user-visible tail was workspace relationship analysis, which did not finish in 10 minutes on Release or 30 minutes on Debug before this work.
- Measurement harness: `relationship_perf_test` builds a project snapshot, preloads workspace symbols, runs workspace relationship analysis, and prints per-stage timings. The harness is intentionally a focused manual perf target rather than a default CTest case.
- Stage effect 1: the initial probe showed a 27-file `test_sv/new` relationship pass at 21.177s, with `snapshot_merge_ms=20540` taking roughly 97% of the time. Indexed snapshot rebind/dedupe reduced that small-workspace merge to roughly 42ms and total time to roughly 0.68s.
- Stage effect 2: the same indexed snapshot work did not make `huge_prj` finish, so per-file profiling was added temporarily and showed misses in `findSymbolRecordByName()` still falling back to the global provider and repeatedly scanning the 96,837-symbol snapshot. Files with little relationship data could still cost seconds because of those all-symbol scans.
- Stage effect 3: disabling the global provider fallback when a published snapshot exists, plus per-run relationship context indexes for name/local-handle/module/containing-module lookup and linear best-candidate selection, dropped the Release `huge_prj` relationship worker to 3.891s.
- Final Release reference for `test_sv/huge_prj`: `relationship_worker_ms=3685`, `relationship_total_ms=3891`, `relationship_extraction_ms=1664`, `relationship_compute_ms=487`, `relationship_snapshot_merge_ms=950`, `relationship_publish_ms=206`, `relationship_files=428`, `relationships=660936`, and `engine_relationships=98409`.
- Final Debug reference for the same relationship path is 13.062s total. Use it only for debugging and sanity; Release is the performance decision build. The Release harness also reports `perf.symbol_ms=70290` for its symbol preload, but that is a benchmark setup cost and should not be compared directly against the GUI symbol-stage number above.
- Verification for this block: Release build of `relationship_perf_test`; manual Release run against `test_sv/huge_prj` completed in 3.891s for the relationship path. Debug `relationship_test` still has the two known pre-existing signal-kernel fixture failures under `test_sv/new`; they are not relationship-performance regressions.
- Next Huge Workspace milestones: keep Release benchmarks as the performance source of truth, decide whether the now-fast full relationship warm-up is acceptable in GUI open-flow measurements, and consider persistent cache or incremental relationship publication only after measuring a new full-open bottleneck.

### Post-L: Huge Workspace Symbol Publication Performance MVP

- Status: implemented in the current worktree.
- Scope: thirty-eighth usable Huge Workspace Mode milestone and the first measured fix for the workspace symbol stage itself. It does not change Slang extraction semantics, diagnostics semantics, staged publication policy, or relationship warm-up policy.
- Telemetry: `WorkspaceAnalysisTelemetry` now records worker, symbol extraction, result assembly, diagnostics extraction, publication, publication update, checkpoint snapshot, final snapshot, and total elapsed time. `relationship_perf_test` now uses `startAnalyzeProjectAsync()` plus an event loop so its symbol-stage timing matches the GUI async publication path instead of the old synchronous benchmark artifact.
- Baseline: Release async `relationship_perf_test` on `test_sv/huge_prj` measured `symbol_ms=46206`, `symbol_worker_ms=1640`, `symbol_extraction_ms=1147`, `symbol_result_assembly_ms=174`, `symbol_diagnostics_ms=317`, `symbol_publication_ms=44560`, `symbol_publication_update_ms=44157`, and `symbol_final_snapshot_ms=381`.
- Finding: the 46s user-visible symbol stage was not Slang-bound. Slang-backed symbol extraction plus diagnostics was roughly 1.6s; the real cost was native-store publication repeatedly scanning existing records for files that had no previous native coverage.
- Fix: `SemanticIndex::replaceNativeSymbolRecordsForFile()` now checks whether the normalized file already has native coverage before scanning `m_nativeSymbolRecords` for old records. First-time workspace publication skips that empty scan, while re-publication of already-covered files still removes stale records.
- Final Release reference for `test_sv/huge_prj`: `symbol_ms=5096`, `symbol_worker_ms=1645`, `symbol_extraction_ms=1151`, `symbol_result_assembly_ms=175`, `symbol_diagnostics_ms=317`, `symbol_publication_ms=3445`, `symbol_publication_update_ms=3043`, `symbol_final_snapshot_ms=377`, `symbols=96837`, and `diagnostics=388`.
- Optimization effect: symbol-stage completion improved from 46.206s to 5.096s, and publication update improved from 44.157s to 3.043s. The same Release run measured relationship total at 4.306s, so the full symbol-plus-relationship completion path is now roughly 9.4s in the benchmark.
- Next Huge Workspace milestones: keep Release async telemetry as the performance source of truth, consider deeper native-store bulk replacement only if the remaining ~3s publication update becomes important, and defer one-pass Slang symbol+diagnostic extraction because current measurements show Slang is no longer the bottleneck.
- Verification for this block: Release build target `relationship_perf_test`; manual Release run against `test_sv/huge_prj`; focused Release `relationship_test` build and run. `relationship_test` still reports the two known real-workspace signal-kernel fixture failures and no new failure class.

### Post-L: COM Mode And Global Control Command Surface MVP

- Status: implemented in the current worktree.
- Scope: editor-local COM Mode starting point plus the Global Control domain restructure. COM Mode is not a Vim clone; it has only INSERT and COM states, enters from Esc whenever an editor tab is open, forwards non-editor-focus Esc through the current editor's existing cancellation/COM path, exits with backtick, keeps Enter for commands that need confirmation, and renders a high-contrast bottom command strip for the whole app.
- Implemented COM g-domain commands: `gm` opens a module picker, `g<num><Enter>` jumps to a module-relative line, `gp` is a non-executable p-family prefix, `gpk` opens a package picker, `gpa` opens current-scope parameter/localparam picker, `gpo` appends a multiline ANSI port row, `gsi` moves to the internal signal declaration insert point, `gii` moves to the instance insert point, `gac` moves to the continuous assign insert point, `gpi` moves to the parameter insert point, `ge` is a non-executable end-family prefix, `gef` moves before the final `endmodule`, and `gsd` opens a current-module signal declaration picker.
- Architecture: `ComModeCoordinator` owns command state, command strip, and picker UI; `ComModeService` owns semantic picker query/report shaping from `SemanticIndexSnapshot`; Tree-sitter/editor structure edits flow through `EditorSyntaxState` and `TSDocument` insert targets before `MyCodeEditor` applies one undoable edit. UI code remains a consumer and does not scan workspace files, run Slang, or use regex for SystemVerilog semantics.
- Global Control now shows root domains only: `ow` and `fd`. `ow` is no longer directly executable and uses `ow <num>` for the number of workspaces to open. `fd` is no longer directly executable and uses `fd r` for Fold Region Mark Mode and `fd s` for Fold Shelf Mode; old flat `fds` is no longer a displayed command.
- Verification for this block: focused Release compile/link targets for `ts_doc_test`, `completion_test`, and `gui_smoke_test`; additional Release compile/link for `gui_smoke_test` after `gsd`; `git diff --check` passed. The Windows test executables were not launched during the final doc/push step because GUI smoke runs had recently produced external application-error dialogs.
