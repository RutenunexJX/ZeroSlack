...
- Ctrl+Space opens a centered ZeroSlack Global Control / Search Everywhere overlay from editor or non-editor focus; search results are shaped by `GlobalControlService` from ProjectModel files, SemanticIndexSnapshot symbols, CodeTemplateService templates, editor actions, RTL Insights entries, settings, and existing command coordinators.
...
- `;:` is reserved and intentionally inactive; editor action discovery and execution currently belongs to Global Control.
