...
- Status: implemented in the current worktree.
- Scope: editor-local COM Mode starting point plus the Global Control domain restructure. COM Mode is not a Vim clone; it has only INSERT and COM states, enters from Esc in editor focus after cancellable editor states are handled, exits with backtick, keeps Enter for commands that need confirmation, and renders a high-contrast bottom command strip for the whole app.
