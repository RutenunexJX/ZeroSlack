...
- I1 is in progress: Slang collection now exposes `collectSymbolRecords`, `SlangManager` exposes `extractSymbolRecords` / `extractWorkspaceSymbolRecords`, and `SymbolAnalyzer` consumes those record-native APIs directly. Remaining I1 work is to shrink the collector internals so `SymbolInfo` is only a private temporary adapter and then disappears from the primary collection path.
