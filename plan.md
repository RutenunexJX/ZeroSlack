...
- I2 is in progress: `SemanticIndex` owns a semantic-native record/content store with file replacement, local handle assignment, stable-key indexes, native content state, native file coverage, native refresh, and native-over-snapshot replacement semantics; snapshot publication now reads records/content from `SemanticIndex` instead of rebuilding records from `sym_list`. Remaining I2 work is to shrink or delete the remaining direct record fallback and database write bridge paths.
