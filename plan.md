...
- I1 is in progress: Slang collection now exposes `collectSymbolRecords`, `SlangManager` exposes `extractSymbolRecords` / `extractWorkspaceSymbolRecords`, and `SymbolAnalyzer` consumes those record-native APIs directly. `slangsymbolcollector` now builds `SemanticSymbolRecord` as its primary output and keeps `collectSymbols` as a temporary private compatibility wrapper. Remaining I1 work is to remove the remaining manager-facing legacy extraction API and shrink or delete the collector reverse adapter.
