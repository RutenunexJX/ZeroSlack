...
- I2 is in progress: `SemanticIndex` owns a semantic-native record/content store with file replacement, local handle assignment, and stable-key indexes; snapshot publication now reads records/content from `SemanticIndex` instead of rebuilding records from `sym_list`. Remaining I2 work is to shrink direct `sym_list` fallback paths and move cached content / file replacement fully behind the native store.
