...
public:
    ...
    bool notify(QObject* receiver, QEvent* event) override {
        ...
        if ((compositor && receiver == compositor && type == QEvent::Paint)
            || (!compositor && receiver == dock && type == QEvent::Resize)) {
...
int main(int argc, char** argv) {
    ...
    if (!editor || !bar || !app.dock) return 7;
    ...
    for (QSize size : sizes) {
        ...
        for (int i = 0; i < 12; ++i) {
            ...
            if (scene == "left") bar->setDisplayMode(i % 2 ? ElaNavigationType::Maximal : ElaNavigationType::Minimal);
            ...
            renderers.append(app.compositor->isActive() ? app.compositor->renderer() : "immediate");
            if (app.compositor->isActive()) {
                deadline.start(3000);
                motion.exec();
            }
            ...
            if (app.compositor->isActive()) return 8;
        }
        QJsonObject events;
        ...
        scenarios.append(QJsonObject{{"scene", scene}, {"width", host.width()}, {"height", host.height()},
            ...
            {"renderer", app.compositor ? app.compositor->renderer() : "live-layout"},
            {"compositionRefreshRate", app.compositor ? app.compositor->compositionRefreshRate() : 0},
