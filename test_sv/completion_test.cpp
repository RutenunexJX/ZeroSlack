...
int main(int argc, char** argv) {
    ...
    waveScopeDocument.setText(scopeInput);
    const int firstAlwaysPosition =
        scopeInput.indexOf(QStringLiteral("always_ff"));
    const int combAlwaysPosition =
        scopeInput.indexOf(QStringLiteral("always_comb"));
    const int pulseAlwaysPosition =
        scopeInput.indexOf(QStringLiteral("always @(posedge"));
    const TSAlwaysScopeTarget firstAlwaysScope =
        waveScopeDocument.alwaysScopeTarget(
            scopeInput.indexOf(QStringLiteral("q <= '0")));
    ...
    waveScopeEditor.setPlainText(scopeInput);
    QTextCursor waveScopeCursor(waveScopeEditor.document());
    waveScopeCursor.setPosition(
        scopeInput.indexOf(QStringLiteral("next = q")));
